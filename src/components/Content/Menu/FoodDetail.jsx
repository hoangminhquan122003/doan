...
const StyledCard = styled.div`
    ...
    background-color: ${props => !props.available ? '#f0f0f0' : '#ffffff'};
color: ${props => !props.available ? '#b0b0b0' : '#333333'};
cursor: ${props => !props.available ? 'not-allowed' : 'pointer'};

&:hover {
    box-shadow: ${props => !props.available ? 'none' : '0 12px 20px rgba(0, 0, 0, 0.15)'};
    border: ${props => !props.available ? '1px solid #e0e0e0' : '3px solid #0D92F4'};
}
    ...
    const handleClick = () => {
        if (!available) {
            Modal.warning({
                ...
                content: "Món ăn này hiện đã hết hàng.",
            });
            return;
        }
    ...
    return (
        <>
            <StyledCard available={available} onClick={handleClick}>
                ...
                {!available && <div className="overlay-unavailable">Hết món</div>}
            </StyledCard>
